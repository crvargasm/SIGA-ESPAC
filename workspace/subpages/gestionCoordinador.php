...
<body>
  ...
  <div id="page">
    ...
    <div class="dashboard">
      ...
      <div class="backc">
        <div id="titles">
          <p>Por favor Ingrese Primer Apellido, Cédula del Catequista o Seleccione la Parroquia a la que pertenece el Coordinador:</p>
        </div>
        <div id="formconsulta">
          <form action="gestionCoordinador.php" method="POST">
            <input type="text" name="apellido" placeholder="Ingrese Primer Apellido">
            <input type="text" name="ID" placeholder="Ingrese C.C. del Coordinador">
            <select name="parroquia">
              <option disabled selected value=0>Seleccione la Parroquia del Coordinador:</option>
              <?php
            ...
            <button type="button" onclick="location.href='gestionCoordinador.php'">Limpiar</button>
          </form>
        </div>
        <div id="consulta">
          <?php
          ...
          if (empty($apellido) && empty($id) && $valueSel == 0 && $boton) {  //Verifica que NO este Vacia La Consulta
            ...
            if (!empty($apellido) && empty($id) && $valueSel == 0 && $boton) {  //Verifica SOLO Primer Apellido
              require '../database.php';
              $result = $conn->query("SELECT * FROM infoCatequista WHERE coordinadorHabilitar=1 AND apellidoCatequista LIKE '$apellido%'");
              $numfilas = $result->num_rows;
            }
            if (empty($apellido) && !empty($id) && $valueSel == 0 && $boton) {  //Verifica SOLO ID del Catequista
              require '../database.php';
              $result = $conn->query("SELECT * FROM infoCatequista WHERE coordinadorHabilitar=1 AND cedulaCiudadania=$id");
              $numfilas = $result->num_rows;
            }
            if ($valueSel != 0 && empty($apellido) && empty($id) && $boton) {  //Verfica SOLO Parroquia Catequista
              require '../database.php';
              $result = $conn->query("SELECT * FROM infoCatequista WHERE coordinadorHabilitar=1 AND parroquiaCatequista=$valueSel");
              $numfilas = $result->num_rows;
            }
            if (!empty($apellido) && !empty($id) && $valueSel == 0 && $boton) { //Verifica Primer Apellido y ID
              require '../database.php';
              $result = $conn->query("SELECT * FROM infoCatequista WHERE coordinadorHabilitar=1 AND cedulaCiudadania=$id AND apellidoCatequista LIKE '$apellido%'");
              $numfilas = $result->num_rows;
            }
            if (empty($apellido) && !empty($id) && $valueSel != 0 && $boton) { //Verifica ID y Parroquia
              require '../database.php';
              $result = $conn->query("SELECT * FROM infoCatequista WHERE coordinadorHabilitar=1 AND cedulaCiudadania=$id AND parroquiaCatequista=$valueSel");
              $numfilas = $result->num_rows;
            }
            if (!empty($apellido) && empty($id) && $valueSel != 0 && $boton) {  //Verifica Primer Apellido y Parroquia
              require '../database.php';
              $result = $conn->query("SELECT * FROM infoCatequista WHERE coordinadorHabilitar=1 AND parroquiaCatequista=$valueSel AND apellidoCatequista LIKE '$apellido%'");
              $numfilas = $result->num_rows;
            }
            if (!empty($apellido) && !empty($id) && $valueSel != 0 && $boton) {  //Verifica TODOS
              require '../database.php';
              $result = $conn->query("SELECT * FROM infoCatequista WHERE coordinadorHabilitar=1 AND cedulaCiudadania=$id AND parroquiaCatequista=$valueSel AND apellidoCatequista LIKE '$apellido%'");
              $numfilas = $result->num_rows;
            }
          }
          ?>
